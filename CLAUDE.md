...
**distDir alternativo fuera de `.gitignore` — envenena a Tailwind 4 [Aug 2026]**
Un build aislado con `E2E_DIST_DIR=.next-s3b` (nombre NO listado en `.gitignore`, que ancla `/.next/` de forma exacta y por eso enumera `/.next-setter/` y `/.next-galeria/` una por una) dejó megabytes de JS minificado a la vista de la auto-detección de fuentes de Tailwind 4, que respeta `.gitignore`. Resultado: `./src/app/globals.css:4:1 — Module not found: Can't resolve './&'` y la ruta entera en 500, **con el CSS intacto**. Despista doble: el error señala un archivo sano, y **borrar `.next` NO lo arregla** mientras el directorio intruso siga existiendo (el orden correcto es borrar el intruso y recién después limpiar el caché). Regla: cualquier `distDir` alternativo se agrega a `.gitignore` ANTES de correr el build, o se usa uno de los nombres ya ignorados. Corolario: un error de build que apunta a un archivo verificablemente correcto es señal de contaminación del scan, no del archivo.

**Pestaña oculta — toda medición de scroll o layout da cero [Aug 2026]**
Con la pestaña ocluida, minimizada o en una ventana de fondo, el navegador saltea los rendering steps: no despacha eventos `scroll`, no corre `requestAnimationFrame` y `window.innerWidth` devuelve 0. Las tres consecuencias se sufrieron en el mismo sprint: una primitiva de scroll parecía rota (el progreso quedaba clavado mientras `scrollY` sí cambiaba), el layout responsive se reportaba en su rama mobile con una ventana de 1440px, y los screenshots fallaban por `clip` degenerado. Regla: verificar `document.visibilityState` (y que `window.innerWidth > 0`) ANTES de confiar en cualquier medición de scroll o de layout. Esto invalida toda verificación automatizada de scroll —`visual-qa` incluido— que no garantice la pestaña visible. Corolario del método: si un fix es correcto en estático pero "falla" en runtime, buscar primero un discriminador empírico del ENTORNO de medición antes de volver a tocar el código.

**R3F v9 escribe `state.pointer` por su cuenta [Aug 2026]**
`<Canvas>` sin prop `eventSource` conecta su sistema de eventos a **su propio div contenedor** y le fuerza `pointerEvents: 'auto'` (`react-three-fiber` v9, `:113` y `:151`). Como un descendiente puede reactivar `pointer-events`, un `pointer-events: none` en cualquier ancestro **no lo frena**: r3f sigue actualizando `state.pointer` al pasar el cursor sobre la caja del canvas. Por eso eliminar un feed de puntero propio (un `PointerSync` sobre `window`) NO desactiva un mouse-follow — solo lo acota a la caja. Para neutralizarlo de verdad hay que forzar `state.pointer` a cero dentro de un `useFrame`, patrón ya usado en `Hero.tsx:164-165` y `BrandedLogoWhite.tsx:53-54`; sirve además cuando la rotación vive en un componente frozen, porque no lo toca.

**`Page.captureScreenshot` congela los pasos de render de la página [Sep 2026]**
Un banco de medición que intercala capturas con estado vivo (`IntersectionObserver`, transiciones de CSS, cualquier cosa que dependa de un rendering step) ve ese estado CLAVADO justo después de cada captura: el observador deja de entregar y las transiciones dejan de avanzar. `requestAnimationFrame` sigue resolviendo, así que el defecto no se nota por ese lado — el doble-rAF que `scrollA` (`scripts-b4/navegador.ts`) usa para confirmar el scroll no lo delata. Se destraba re-mandando `Emulation.setDeviceMetricsOverride` (o sea, llamar `emular()` de nuevo) después de cada `capturar()`. Es el primo del defecto de la pestaña oculta de más arriba: los dos apagan rendering steps por una razón distinta y con la misma forma de sufrirse — una medición intercalada con capturas da un estado que no es el del sitio.

**Una cadena de `transitionend` se cuelga si dos etapas seguidas comparten el valor [Sep 2026]**
Una máquina de estados que avanza por `transitionend` (en vez de por temporizador, para no duplicar la duración en dos lugares) asume que CADA paso cambia la propiedad que dispara el evento. Con dos etapas consecutivas que declaran el MISMO valor de esa propiedad (acá: dos `clip-path` iguales, el cuadro entero cubierto), la transición entre ellas no tiene nada que animar, `transitionend` nunca dispara, y la máquina queda clavada en la primera de las dos para siempre — sin error, sin log, solo inmóvil. Regla: en una cadena por eventos, verificar que CADA transición consecutiva de estados cambia el valor observado; si dos etapas comparten el valor por diseño, la transición tiene que saltar derecho a la SIGUIENTE etapa con un valor distinto, no encadenar por la etapa muerta.

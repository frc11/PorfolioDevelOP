/**
 * B11 · EL CENSO DE ACONTECIMIENTOS DE B2 — el gate de ritmo, antes y después
 * de mover el texto.
 *
 *     npx tsx scripts-b11/censo.ts antes
 *     npx tsx scripts-b11/censo.ts despues
 *
 * Es `scripts-b9/b9-censo.ts` letra por letra —el instrumento de
 * `scripts-b4/censo.ts` con el paso de 120 px y los dos anchos de la
 * coreografía— con una sola diferencia: el ORIGEN, que allá era el 3002 de
 * `C:\v3-sincronia` y acá es el 3000 de este worktree. Un censo contra otro
 * puerto sería el censo de otro árbol.
 *
 * ⚠️ **La vara.** B9 dejó el hueco máximo en **1,33 pantallas a 1920 y 1,20 a
 * 1440** (`B9-DELTAS.md` §5), medido con ESTE instrumento. Mover un bloque de
 * altura mueve su acontecimiento: la cifra de «antes» de esta corrida es la que
 * vale para restar contra la de «después», y las dos se comparan contra 1,33 y
 * 1,20.
 */
...
import { ORIGEN, dos, jsonPendiente, mudarPendientes } from './b11-comun'
...
async function medirUno(perfil: Perfil): Promise<Record<string, unknown>> {
  const chrome = await lanzarChrome({ perfil: perfilDeChrome('b11-censo'), ancho: perfil.ancho, alto: perfil.alto + 120 })
  try {
...
async function main(): Promise<void> {
  ...
  jsonPendiente(`censo-${ETIQUETA}`, {
    etiqueta: ETIQUETA,
    instrumento: 'scripts-b4/censo.ts (el de B2-DELTAS §0, corrido por b9-censo), paso 120 px, 8 esperas de 140 ms, sin estrangular, emulado, origen 3000',

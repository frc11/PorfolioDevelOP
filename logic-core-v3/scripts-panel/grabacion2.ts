/**
 * LA GRABACIÓN DE TU PANEL (SPRINT 2), A 1440.
 *
 *     npx tsx scripts-panel/grabacion2.ts
 *
...
const SALIDA = 'C:/Users/Valentino/.cache/b4-medicion/panel/grabacion2'
...
async function principal(): Promise<void> {
  ...
  try {
    ...
    await medir<number>(p, `(async () => { const s = document.querySelector('[data-pieza=caos-del-panel]'); window.scrollTo(0, s.getBoundingClientRect().top + scrollY - 950); await new Promise((r) => setTimeout(r, 2500)); return 1 })()`)
    ...
    await esperar(800)
    console.log('entra el título y arranca el caos')
    await rueda(p, 10, 220)
    await esperar(1500)
    ...
    await esperar(3000)
    console.log('subiendo: el remate se va')
    await rueda(p, 14, 260, -1)
    await esperar(2500)

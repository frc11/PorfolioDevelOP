...
function ColorController({
    ...
    useFrame((state, delta) => {
        ...
        if (phase === 'dormant') {
            ...
            // CONTEXTO ACTIVO: tomar el color de la secci├│n
        ...
        // M├ís lento durante una transici├│n de contexto fresh
...
function Eyebrow({
    ...
    useFrame((state, delta) => {
        ...
        if (isSleeping) {
            ...
            // Ceja izquierda sube ligeramente (curiosidad/concentraci├│n humana)
            if (isLeft) {
                targetY = 0.27;
                expressionRot = 0.08;
            } else {
...
function Mouth({
    ...
    useFrame((_, delta) => {
        ...
        if (isSleeping) {
            ...
            targetSX = 0.65;   // boca peque├▒a
            targetSY = 0.18;   // l├¡nea sutil
            targetSZ = 0.65;
...
function OrbitalParticles({
    ...
    // 24 part├¡culas en ├│rbitas el├¡pticas con inclinaciones distintas
    const { positions, speeds, radii, inclinations } = useMemo(() => {
        ...
        for (let i = 0; i < count; i++) {
            // ├üngulo inicial distribuido uniformemente
            const angle = (i / count) * Math.PI * 2;
            // Radio de ├│rbita: entre 1.05 y 1.35 (fuera del blob)
            const r = 1.05 + Math.random() * 0.3;
            // Inclinaci├│n del plano de ├│rbita (da sensaci├│n 3D)
            ...
            // Velocidad angular individual (algunas m├ís lentas, otras m├ís r├ípidas)
    ...
    useFrame((state, delta) => {
        ...
        // Velocidad global de ├│rbita seg├║n estado
        ...
        // Tama├▒o de part├¡cula seg├║n estado
    ...
    useFrame((state, delta) => {
        ...
        if (faceGroupRef.current) {
            ...
            if (isSleeping) {
                ...
                // Limitamos el rango de rotaci├│n para que no se vea raro
                const MAX_YAW = 0.55;   // rotaci├│n Y izquierda/derecha
                const MAX_PITCH = 0.32; // rotaci├│n X arriba/abajo
                ...
                // Micro-offset de la cara en X para dar sensaci├│n de peso
                gazeOffsetX = -mx * 0.04;

                // Agregamos un micro-drift sinusoidal muy sutil para que no se
                // vea mec├ínico cuando el mouse est├í quieto
                gazeTargetY += Math.sin(time * 0.4) * 0.04;
                gazeTargetX += Math.cos(time * 0.32) * 0.02;

                // Reacci├│n de mensajes recibidos/enviados suma al movimiento
...
export function LegacyNeuroAvatar({
    isThinking = false,
    showPrompt = false,
    isBooped = false,
    isHovered = false,
    ...
    return (
        ...
        >
            ...
            >
                ...
                {/* KEY LIGHT ÔÇö principal, posicionada arriba-derecha-frente
                    Da forma y define la expresi├│n del rostro */}
                <pointLight
                    ref={keyLightRef}
                ...
                {/* FILL LIGHT ÔÇö izquierda-abajo, suaviza las sombras
                ...
                {/* RIM LIGHT ÔÇö atr├ís-arriba, separa el blob del fondo
                    El efecto "producto Apple" viene de ac├í */}
                <pointLight
                    position={[0, 3.5, -3]}
                    intensity={3.2}
                ...
                {/* FACE LIGHT ÔÇö muy cerca, ilumina los rasgos faciales
            ...
            {/* Sleep Z indicator - SOLO si est├í sleeping */}
            {isSleeping && (
                <div
                    aria-hidden="true"
                    style={{

...
export const showWhatsappHandoffInputSchema = z.object({
  prefilledMessage: z.string().min(20).max(500).describe(
    'Mensaje completo pre-llenado para WhatsApp. Este es el TEXTO QUE EL VISITANTE VA A MANDAR al WhatsApp del negocio al clickear la card. DEBE incluir: saludo breve, nombre del visitante si lo dio, y resumen de qué busca (1-2 oraciones). Tono: español rioplatense informal. Ej: "Hola, soy Juan. Estoy buscando un Corolla XEi 0KM y quiero coordinar cuándo lo retiro."'
  ),
  ...
  topicSummary: z.string().min(15).max(400).describe(
    'Resumen estructurado en 1-3 oraciones de QUÉ BUSCABA el visitante y QUÉ SE HABLÓ. Va al dashboard del dueño y al log. Ejemplo: "Visitante interesado en Corolla XEi 0KM, preguntó por entrega y financiación. Mencionó tener un usado para parte de pago."'
  ),

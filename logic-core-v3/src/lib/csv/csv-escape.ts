/**
 * Copiado de src/modules/chatbot/server/leads/csv/ por frontera de modulo
 * (no se puede importar desde app/admin). Si cambia la logica de escape,
 * sincronizar ambas copias o extraer a un compartido (decision diferida).
 *
 ...
 *    `=HYPERLINK(...)` en su nombre o mensaje para exfiltrar datos cuando el
 *    dueño abre el CSV. Mitigación: prefijar con comilla simple (`'`) —
 *    Excel/Sheets dejan de tratarlo como fórmula pero el valor visible al
 ...
 * el `'` queda FUERA de las quotes y rompe el parseo del CSV.
 */

 ...
 */
export function ContenidoDelCierre({ anima }: { readonly anima: boolean }): React.JSX.Element {
  return (
    <Pie>
      <EncabezadoDeSeccion id="cierre" nombre={ETIQUETA_DE_SECCION} />

      <Bloque
        anclas={PATRONES.P1.anclas}
        inerciaSegundos={inerciaDe(PATRONES.P1)}
        anima={anima}
      >
        {(progreso) => (
          <CanalDeTitular
      ...
      <Bloque
        anclas={PATRONES.P2.anclas}
        inerciaSegundos={inerciaDe(PATRONES.P2)}
        anima={anima}
      >
...
/**
 * LA SECCIÓN. Recibe `anima` y NADA MÁS: no consulta la compuerta.
 *
 * Es lo que permite que el instrumento renderice las DOS ramas sin inventar un
 * atributo de forzado en el producto, y es lo que hace verificable la regla de
 * que abajo de 1025 no se monta ni un hook del sistema de motion.
 */
export function Cierre({ anima }: { readonly anima: boolean }): React.JSX.Element {
  return (
    <Seccion id="cierre">
      <ContenidoDelCierre anima={anima} />
    </Seccion>
  )
}

/** El envoltorio de compuerta. Es lo que usa la ruta. */
export function CierreConCompuerta(): React.JSX.Element {
  const anima = useAnima()
  return <Cierre anima={anima} />
}

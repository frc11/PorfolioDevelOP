...
const quieto = renderToStaticMarkup(<Servicios anima={false} />)
const animado = renderToStaticMarkup(<Servicios anima={true} />)
...
const CODIGO = quitarComentarios(
  [...ARCHIVOS, 'src/app/v3/secciones-b/_contrato/Bloque.tsx'].map(leer).join('\n'),
)
...
afirmarIgual(cuentaDeAtributo(animado, 'data-hueco'), 1, 'un hueco de medio por tramo, con su marcador y su sizes')
...
afirmar(heredadas.length <= 6, `no las agrega esta sección: ${heredadas.length} sobre ${elementos} elementos`, 'dos por servicio, de EtiquetaDeSeccion y de HuecoDeMedio')
controlPositivo('cn() SÍ descarta el tamaño cuando entra un color por className', ['font-cuerpo', 'text-fluido-micro', 'leading-micro', 'tracking-micro', 'font-normal', 'text-tinta-media uppercase'], (partes) => cn(...partes).split(' ').includes('text-fluido-micro'))
controlPositivo('  y la familia y el peso cuando entra otra familia', ['font-cuerpo', 'text-fluido-caption', 'leading-texto', 'tracking-texto', 'font-medio', 'font-codigo uppercase'], (partes) => { const r = cn(...partes).split(' '); return r.includes('font-cuerpo') && r.includes('font-medio') })

...
import { pantallasDe, seccionDe } from '../_contrato/secciones'
...
const QUIETO = renderToStaticMarkup(<TuPanel anima={false} />)
const ANIMADO = renderToStaticMarkup(<TuPanel anima={true} />)
...
controlPositivo('el buscador ve un patrón que no uso', 'const x = PATRONES.P5', (c) => patronesNombrados(c).length === 0)
controlPositivo('y no se come lo que dice un comentario', '/* acá menciono PATRONES.P7 */', (c) => patronesNombrados(c).length > 0)
...
afirmarIgual(valorDe(QUIETO, 'data-relacion'), RELACION_DE_LA_CAPTURA, '  con el valor que viene del dato')
...
// ═══════════════════════════════════════════════════════════════════════════
titulo('10 · Cuántas pantallas ocupa — lo propio se afirma, lo heredado se publica')

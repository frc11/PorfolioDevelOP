...
import {
  ...
  divergenciasDeSuperficie,
  pantallasDe,
  ritmoDelLane,
  seccionesDelLane,
} from '../_contrato/secciones'
import {
...
const secciones = seccionesDelLane()
afirmarIgual(secciones.map((s) => s.id), [...ORDEN_DE_SECCIONES_B], 'las cuatro secciones existen en `_lib/secciones.ts` y están en orden')
...
for (const id of ORDEN_DE_SECCIONES_B) {
  const enLaTabla = secciones.find((s) => s.id === id)?.superficie
  const contrato = SUPERFICIE_DE_CONTRATO[id]
  const marca = enLaTabla === contrato ? ' ' : '≠'
  console.log(`    ${marca} ${id.padEnd(18)} contrato ${contrato.padEnd(20)} tabla ${String(enLaTabla)}`)
}

const divergencias = divergenciasDeSuperficie()
console.log(
  `  ${divergencias.length} divergencia(s). La tabla la escribe el LANE A y este lane no la toca: ` +
    'la superficie es heredada y se publica (regla 13).',
)
afirmar(
  divergencias.every((d) => SUPERFICIE_DE_CONTRATO[d.id] !== undefined),
  'toda divergencia tiene su fila en la tabla acordada — el delta es legible, no un misterio',
  divergencias.map((d) => `${d.id}: ${d.enLaTabla} → ${d.contrato}`).join(' · ') || 'ninguna',
)

// ═══════════════════════════════════════════════════════════════════════════
titulo('5 · El ritmo — pantallas declaradas y momentos reales')

const ritmo = ritmoDelLane()
for (const f of ritmo.filas) {
  console.log(
    `  ${f.id.padEnd(18)} ${f.pantallas.toFixed(1)} pantallas · ${f.pantallasPinneadas.toFixed(1)} pinneadas · ` +
      `${f.secuencias} secuencia(s) → ${f.momentos.toFixed(1)} momentos`,
  )
}
console.log(
  `  TOTAL del lane: ${ritmo.pantallas.toFixed(1)} pantallas declaradas · ${ritmo.momentos.toFixed(1)} momentos reales`,
)
console.log(
  `  la referencia, HOME ENTERA: ${RITMO_DE_LA_REFERENCIA.momentosReales} momentos en ` +
    `${RITMO_DE_LA_REFERENCIA.pantallasNominales} pantallas (${RITMO_DE_LA_REFERENCIA.fuente}).` ,
)
console.log('  ⚠️ Son CUATRO secciones contra OCHO: la comparación es parcial por construcción.')
...
afirmarIgual(ritmo.filas.filter((f) => f.secuencias > 0).length, 1, 'exactamente una sección es una secuencia pinneada')

controlPositivo('el lector de alturas rechaza una unidad que no es svh', '100vh', (alto) => {
  pantallasDe(alto)
  return true
})

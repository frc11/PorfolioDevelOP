/**
 * LOS RECIBOS DEL PRESUPUESTO DE PESO DE `/v3` — PRIMERA MITAD: el techo, y los
 * tres montajes que lo subieron midiendo cada uno en SU worktree.
 *
 * Salen de `s5-presupuesto.ts` en B10, cuando ese archivo llegó a **356 líneas**
 * al resolverse a mano el merge de las cuatro ramas y cruzó el límite de 300 del
 * repo. **El texto se mudó; las constantes se quedaron.** Allá viven los siete
 * números, cada uno con un docblock corto y un puntero acá; acá vive el recibo
 * COMPLETO de cada uno —la medición, lo que se achicó antes de subirlo, la
 * alternativa descartada y el reparto por dueño—, que es lo que hace que cada
 * línea del presupuesto sea **revocable** y la razón por la que el presupuesto
 * se partió en constantes con nombre en vez de un número único.
 *
 * ⚠ Los recibos no entran en 300 líneas, y la regla del repo es partirlos, no
 * recortarlos. El corte es **por dueño y en dos**, y cae donde las ramas se
 * juntaron: acá los tres cuya cifra se midió en su propio worktree y no volvió a
 * moverse —**B4-A, B7 y B6-A**—, más el recibo del techo de 60 KiB; en
 * `s5-presupuesto-recibos-del-merge.ts`, los tres que el merge puso en
 * discusión —**B9, B8 y el heredado**.
 *
...
/**
 * ═══ EL ENCABEZADO ORIGINAL DEL PRESUPUESTO, CONSERVADO ENTERO ════════════
 *
 * ⚠ Los dos lanes escribieron el MISMO párrafo de origen con distintas palabras
 * —B7 en el suyo, B6-A en el suyo— y el merge los dejó a los dos, uno abajo del
 * otro. Se conservan **los dos**: cada uno dice algo que el otro no dice, y
 * ninguna palabra de un recibo se tira.
 *
 * ── El párrafo tal como lo escribió B7 ────────────────────────────────────
 *
 ...
 *
 * ── El mismo párrafo tal como lo escribió B6-A ────────────────────────────
 *
 ...
 *
 * ── Y la advertencia que los dos firmaron ─────────────────────────────────
 *
...
/**
 * ═══ RECIBO DE `PRESUPUESTO_DEL_LANE_KIB` — EL TECHO DE 60 KiB ════════════
 *
 ...
 * ── Por qué DOS constantes y no una sola más grande ───────────────────────
 *
 * El montaje de la marca, medido sobre el árbol de B6-A. El día que alguien
 * atribuya el desvío de HEAD entre los dos entornos y lo devuelva, **el techo
 * baja solo** al borrar esa línea, sin tener que re-derivar nada.
 */

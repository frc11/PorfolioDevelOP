...
const cruces: string[] = []
for (const id of IDS_DE_S6) {
  const ajenas = IDS_DE_S6.filter((otra) => otra !== id).map((otra) => NOMBRES_DE_CARPETA[otra])
...
const importsRevisados = IDS_DE_S6.flatMap((id) =>
 ...
 *
 * ⚠ Se busca también en el CONTRATO, no sólo en las cuatro carpetas de este
 * sprint. Las clases del acento viven en `_contrato/acento.ts` desde que el
 * contrato es uno solo, y una sección que las consume por su nombre
 * (`CLASES_DE_ACENTO.texto`) no escribe la cadena. Buscar sólo en las carpetas
 * daba cero, que es exactamente lo que este contrapeso existe para distinguir
 * de "no hay acento".
 */
const codigoConElContrato = [codigoDelProducto, leer(`${CARPETA_DE_CONTRATO}/acento.ts`)].join('\n')
const usosDelAlias = (codigoConElContrato.match(/\b(?:bg|text|border|fill|stroke)-acento\b/g) ?? []).length
...
/**
 * ⚠ Se cuentan los SALTOS, igual que `wc -l`, y no los pedazos de `split`.
 *
 * `split('\n').length` cuenta uno de más en todo archivo que termina en salto,
 * o sea en todos. Mientras los archivos estuvieron lejos del límite dio igual;
 * cuando dos llegaron a exactamente 300 los reportó como 301, y una comprobación

/**
 * LA PLOMERÍA DE LOS INSTRUMENTOS DE SITIO-S6 — leer archivos y recorrer.
 *
 * ⚠ **Qué cambió en SITIO-S7.** Esto era la plomería del LANE B, que vivía
 * entero bajo `secciones-b/`. La integración borró los lanes: las ocho
 * secciones viven juntas en `_secciones/` con UN contrato. Lo que este archivo
 * declara ahora son **las cuatro carpetas que SITIO-S6 escribió**, que es lo
 * único de lo que sus instrumentos pueden responder (regla 13: se afirma lo
 * propio). El contrato compartido y la composición los mira `s7-contrato`.
 *
...
/** La raíz de las ocho secciones, relativa al proyecto y con barras normales. */
export const LANE = 'src/app/v3/_secciones'
...
export const CARPETAS_DE_SECCION: Readonly<Record<string, string>> = {
  servicios: `${LANE}/servicios`,
  'tu-panel': `${LANE}/tu-panel`,
  'por-que-develop': `${LANE}/por-que-develop`,
  cierre: `${LANE}/cierre`,
}

/** Los ids de las cuatro, en el orden del recorrido. Era `ORDEN_DE_SECCIONES_B`
 *  y vivía en el contrato del lane; el contrato ya no tiene lanes, así que la
 *  lista de lo que ESTE sprint construyó vive con sus instrumentos. */
export const IDS_DE_S6: readonly string[] = [
  'servicios',
  'tu-panel',
  'por-que-develop',
  'cierre',
...
/**
 * Los archivos de las CUATRO carpetas de este sprint, más sus instrumentos.
 *
 * Recorría `secciones-b/` entero. Esa carpeta ya no existe y recorrer la raíz
 * de las ocho vería también las cuatro del otro sprint: se recorre lo propio.
 */
export function archivosDelLane(): string[] {
  const carpetas = [...Object.values(CARPETAS_DE_SECCION), CARPETA_DE_INVARIANTES]
  return carpetas.flatMap((c) => recorrer(c)).sort()
}
...
/**
 * LA SUPERFICIE QUE LA INSTRUCCIÓN DE SITIO-S6 ACORDÓ, sección por sección.
 *
 * ── Qué era y qué es ahora ───────────────────────────────────────────────
 *
 * Era `SUPERFICIE_DE_CONTRATO`, en el contrato del lane B, y existía porque
 * había DOS tablas: la del sitio, que escribía el lane A, y la acordada en la
 * instrucción. Mientras los lanes corrían en paralelo las dos podían diferir, y
 * la regla 13 mandaba publicar el delta en vez de afirmarlo.
 *
 * **Con una sola tabla el delta deja de ser heredado y pasa a ser un error**, y
 * la comprobación pasa de publicar a AFIRMAR: la tabla del sitio tiene que
 * decir exactamente esto. Es lo que compra la integración — una comprobación
 * que sólo miraba se convierte en una que falla.
 *
 * Se queda con los instrumentos y no vuelve al contrato a propósito: es lo que
 * la instrucción acordó, o sea la vara contra la que se mide el dato, y una
 * vara que viva en el mismo archivo que el dato no mide nada.
 */
 ...
 *
 * **Vive acá y no en `_contrato/escaneo.ts` por una razón concreta:** contiene

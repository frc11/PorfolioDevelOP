...
/**
 * El bloque P2 con su única pieza, y su rama quieta.
 *
 * Existe aunque haya UNA sola instancia: sin él, los hijos —la bajada y el
 * CTA— habría que escribirlos dos veces, una por rama, y dos copias del mismo
 * contenido son dos oportunidades de que una se desvíe. Acá la única diferencia
 * entre las dos ramas es quién envuelve.
 */
function SubeEntero({
  estado,
  className,
          ...
          <Grilla columnas="lateral">
            ...
            <Grilla columnas={GEOMETRIA.columnasTotales}>
              <div className={cn('flex flex-col gap-8', GEOMETRIA.claseDeLaMedida)}>
                ...
                <BloqueDeSeccion patron="P2" piezas={GEOMETRIA.piezasDelBloqueDeEntrada}>
                  {(estado) => (
                    <SubeEntero estado={estado} className="flex flex-col items-start gap-6">
                      <BajadaYCta />
                    </SubeEntero>
                  )}
                </BloqueDeSeccion>

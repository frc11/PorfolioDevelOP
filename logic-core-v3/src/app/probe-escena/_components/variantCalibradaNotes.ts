...
/**
 * LOS COMENTARIOS DEL RECORRIDO CALIBRADO A MANO — el que vive en
 * `variantCalibrada.ts` desde S9.
 *
 * Es la mitad de arriba del par de siempre: **datos en un archivo, razonamiento
 * en otro**, con el exportador uniendo los dos. Lo único que S9 cambió es a qué
 * recorrido pertenecen: hasta S8 estos textos eran los de `CHOREO_KEYFRAMES`, y
 * cuando el mix definitivo ocupó ese lugar vinieron acá con su recorrido.
 *
 * ── Por qué es un archivo que casi no tiene contenido propio ───────────
 *
 * Porque la tabla de comentarios pasó de 500 líneas en S7 y se partió **por la
 * mitad del recorrido**: `choreographyNotesFrontal.ts` de la entrada a Números
 * —donde la cámara vive en azimut 0— y `choreographyNotesGiro.ts` de Portfolio
 * al cierre, que es la mitad que gira. Acá quedan el doc del array, los
 * separadores de tramo y la unión de las dos mitades, que es de donde el
 * exportador lee.
 *
 * ⚠️ **La clave es el `name` del keyframe, no su posición.** Renombrar una
 * pose en `variantCalibrada.ts` sin moverla acá le tira el comentario en el
 * próximo export sin que nadie se entere.
 */
 ...
 * peor que ninguno.
 */
export const CALIBRADA_DOC: readonly string[] = [
 ...
 * emitido igual.
 */
export const CALIBRADA_NOTES: KeyframeNotes = { ...NOTES_FRONTAL, ...NOTES_GIRO }

/**
 * Separador de tramo, indexado por el `name` del keyframe que lo SIGUE.
 *
 * Ojo con una asimetría real de este recorrido: los separadores NO caen en los
 * bordes de `CHOREO_TRAMOS`. El keyframe "hero" está en `at` 0,125 —que es el
 * borde del tramo 2— y sin embargo vive bajo el título del tramo 1, porque es
 * la LLEGADA del tramo 1. La asociación es de autor, no derivable, y por eso
 * está escrita a mano acá.
 *
 * Los títulos dicen "Portfolio" y "Movimiento final + cierre" porque así se
 * llamaban los tramos cuando este recorrido se calibró. S9 los renombró a
 * `trabajos` y `cierre` en `CHOREO_TRAMOS`; acá se dejan como estaban, que es
 * lo que corresponde a material de referencia.
 */
export const CALIBRADA_SECTIONS: Readonly<Record<string, ChoreoSection>> = {
  'entrada · mirada alta': {
    title: 'Tramo 1 · Hero',

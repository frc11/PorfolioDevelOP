...
function buildProjectRevalidationPaths(input: {
  ...
  const paths = [
    ...
    `/admin/projects/${input.projectId}/tasks`,
    `/admin/projects/${input.projectId}/hours`,
    `/admin/projects/${input.projectId}/payments`,
  ]

  if (input.leadId) {
    paths.push('/admin/leads', `/admin/leads/${input.leadId}`)
  }
...
function serializeProjectListItem(project: ProjectListRecord) {
  const normalizedServiceType =
    ...
    null
  const isInternal = project.organization?.slug === 'develop'
  const businessName = isInternal
    ? 'Proyecto interno de develOP'
    : (project.organization?.companyName ?? 'Proyecto interno de develOP')

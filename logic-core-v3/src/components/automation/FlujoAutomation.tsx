            ...
            <g key={edge.id}>
              ...
              {isActive && (
                <line
                  x1={from.x} y1={from.y}
                  x2={to.x} y2={to.y}
                  stroke={`rgba(${from.colorRgb}, 0.15)`}
                  strokeWidth={6}
                  strokeLinecap="round"
                />
              )}
              <line
                x1={from.x} y1={from.y}
                x2={to.x} y2={to.y}
                stroke={isActive ? `rgba(${from.colorRgb}, 0.55)` : 'rgba(255, 255, 255, 0.07)'}
          ...
          <div className="flex flex-col items-center md:items-start justify-center gap-6">
            {!isSimulating ? (
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                className="group relative inline-flex items-center gap-4 bg-gradient-to-br from-[#f59e0b] to-[#f97316] text-[#070709] px-10 py-5 rounded-full font-black font-mono tracking-[0.08em]"
              style={{ boxShadow: '0 0 48px rgba(245,158,11,0.35), 0 8px 24px rgba(0,0,0,0.4)' }}
              >
        ...
        <div className="flex flex-wrap gap-3 md:gap-4 mt-12 md:mt-16">
          {performanceMetrics.map((m, i) => (
            <motion.div
              ...
              className="relative overflow-hidden flex-1 min-w-[200px] bg-white/[0.02] rounded-2xl p-6 lg:p-8 text-center"
              style={{
                border: `1px solid rgba(${m.colorRgb}, 0.14)`,
                boxShadow: `0 4px 20px rgba(0,0,0,0.2), 0 0 0 1px rgba(${m.colorRgb}, 0.05)`,
              }}
            >
              ...
              <div className="w-10 h-10 rounded-xl mb-3 mx-auto flex items-center justify-center text-xl"
                style={{ background: `rgba(${m.colorRgb}, 0.1)`, border: `1px solid rgba(${m.colorRgb}, 0.2)` }}
              >
                {m.icon}
              </div>
              <p

...
- ~~**Un arco ligado al progreso.** Meseta a luz plena hasta el final de Números, un escalón chico en Trabajos y Demos, la caída real en el cierre, y la última pantalla en penumbra.~~ **REESCRITO EN B8: el arco es una tarde, UNA NOCHE y una mañana** (`escena/lightArc.ts`, con `s20-arco.invariant.ts` custodiándolo nudo por nudo). La meseta a luz plena sigue hasta que Trabajos toca el pie del cuadro (p=0,4688) —a una sección que alguien está leyendo no se le baja la luz—; en **la pantalla de scroll en que Trabajos entra, atardece** (nivel 1 → 0,08, elevación 36° → 2,7°); **la noche dura exactamente el pin de Trabajos** (la meseta de B4-A, a oscuras: el papel del piso baja de 249 a ~33 sobre 255 a plena key, sin un solo velo); **amanece escondido**, entre que Trabajos se va y la escena vuelve a dibujar para el diferencial (0,625 → 0,7375, donde `visibilidad.ts` suspende el lazo); y el diferencial entra con el sol subiendo hasta **la luz de S9 en su ancla (0,643 en p=0,8525), que se sostiene hasta el final**: la sala no vuelve a bajar detrás del pie. Cada parada del arco es un nudo del anclaje o de la visibilidad, LEÍDO y no copiado.
  - **Por qué la oscuridad la pone la luz y no un velo.** B6-A abrió Trabajos y el Cierre con un fondo oscuro en gradiente sobre la sala, y el humano lo grabó: *un velo oscuro sobre una sala de papel blanco da GRIS, nunca negro*. Con `nivel = sin(elevación)/sin(36°)` como única palanca, la misma sala a nivel 0,08 se lee oscura de verdad (luminancia media medida 0,024–0,035 en el pin, contra 0,0010 de la referencia y 0,037–0,067 de sus pantallas con texto — `B8-LUZ.md`). El velo y sus tres tokens se borraron.
  - **El contraluz se ata a la sala por debajo de 0,34.** La fórmula de S6 vale intacta de 0,34 a 1 —nada de lo que S6–S12 midieron se mueve—, y por debajo cae linealmente a cero: un contraluz que no baja con la sala la lava de gris. 0,34 es el nivel más bajo que el arco viejo alcanzaba, así que es continuo con todo lo medido.
  - **La sombra más larga es la de la noche, no la del cierre**, y el shadow map la alcanza: `SHADOW_FAR` subió de 64 a 200 porque con un sol a 2,7° la sombra del logo mide 168 unidades (`s7-sol` §3 y `s20-arco` §5 lo afirman; con 64 no entraba).
  - **Las partículas brillan en la noche, sin color.** De día se leen por ser más oscuras que el papel; de noche eso es nada. Un uniform que sigue al nivel del arco las lleva a un gris parejo (0,25 lineal, tres canales iguales) por debajo de 0,34, y es exactamente cero con luz. La cantidad es la del campo de siempre. `particleGlow.ts`, `s20-brillo.invariant.ts`.
- **El sol barre 180°, y ese barrido lo fijó el recorrido.** Con la cámara orbitando 360° y leyendo contenido en seis azimuts, el ángulo entre la luz y el observador recorre media vuelta sí o sí: el contraluz cae en algún lado. La recta de azimut de S9 (−42° → 115° entre 0,125 y 0,5) no se movió con B8, ni los azimuts de sus nudos. El sol entra en cuadro al empezar Demos y desde ahí la escena va a contraluz.
- ~~**El arco es una TARDE, no un día entero.**~~ Era cierto para la forma que se evaluó entonces —arrancar con el sol rasante y subirlo hasta Números dejaba el hero más oscuro que su cierre—. B8 no arranca de noche: **arranca a mediodía, pone la noche en el medio (Trabajos) y cierra con una mañana más baja que la tarde** (22,2° contra 36°). La objeción sigue valiendo para el hero, que no se tocó.
- **El apagado no es plano.** El ambiente muere antes que las fuentes y el contraluz se resiste: la sala **gana contraste** al apagarse en vez de volverse gris. Desde B8, además, el contraluz se apaga con la sala por debajo de 0,34 (arriba).
...
50. ✅ **B7 — LOS DEFECTOS ABIERTOS: `prefers-reduced-motion` SE HONRA, Y DOS DEFECTOS SE CIERRAN SIN ARREGLAR PORQUE LA MEDICIÓN LOS DESMIENTE (2026-09-07).**

    Rama `v3/defectos`, worktree `C:\v3-defectos`, en paralelo con la sesión de la escena persistente. **La zona prohibida quedó intacta**: ni un byte en `_secciones/trabajos/`, `_secciones/cierre/`, el pie, `superficies.ts`, `_lib/escena/` ni `secciones.ts`.

    ── 🔴 **`prefers-reduced-motion` — EL DEFECTO MÁS GRAVE DEL INVENTARIO, CERRADO CON SUS DOS MITADES**

    **La primera mitad, medida en el navegador:** con la preferencia emulada por `Emulation.setEmulatedMedia` y verificada con `matchMedia` desde la página, el barrido acumulado pasó de **2.450 transformadas contra 2.450** a **2.450 sin la preferencia contra 0 con ella**, y de 5 piezas de texto partido a 0. **Y el contenido no se apagó**, que es la mitad que se olvida: 7.115 caracteres visibles con la preferencia contra 7.104 sin ella, y **cero nodos de texto en opacidad efectiva 0** en las dos ramas. `scripts-b7/a-reducido.ts` → `docs/rediseno/outputs/b7/a-reducido.json`.

    **La causa, y por qué el arreglo es UNA LÍNEA DE ARQUITECTURA Y NO UNA LÍNEA DE CÓDIGO.** `useMovimientoReducido()` era `useReducedMotionConfig() ?? false`, y ese hook de `motion` corta en `MotionConfigContext`, cuyo default es textual `reducedMotion: "never"`. En todo `/v3` no había un solo proveedor, así que el media query **nunca se llegaba a leer**. Lo que el repo ya tenía era la política que sí funciona —`_lib/usePrefiereMenosMovimiento.ts`, `useSyncExternalStore` sobre el media query, con snapshot de servidor `true`—, de donde leen el cursor y el scroll suave. **O sea que había DOS políticas de movimiento reducido y la que no llegaba era la del sistema de motion.** `v3/layout.tsx` monta ahora `ProveedorDeMovimiento`, que traduce esa única política al vocabulario de `MotionConfig`. **No se escribió una tercera**, y los docblocks de `reducido.ts` que afirmaban lo contrario se reescribieron con la causa.

    **La segunda mitad: el invariante se FORTALECIÓ, de 46 afirmaciones a 76.** `reducido.invariant.tsx` estaba **verde por arnés** —renderizaba a través de `<MotionConfig reducedMotion={preferencia}>` y después afirmaba sobre esa misma `preferencia`, o sea que inyectaba la entrada bajo prueba—. R1…R6 quedaron **enteras, carácter por carácter**; lo que se agregó es la segunda mitad por otro camino (`reducido-produccion.tsx`): que el árbol de producción monta el proveedor, que **sin el proveedor el mismo árbol vuelve a animar** —el defecto de hoy capturado como control negativo—, y que las tres lecturas de la preferencia del sitio salen del mismo hook. La tercera mitad la pone el navegador, y es la de arriba.

    ⚠️ **EL PRECIO, MEDIDO, Y ES UNA DECISIÓN ABIERTA:** el arreglo cuesta **1,92 KiB crudo propios** en la carga inicial de `/v3` —chunk por chunk contra el build de B4-B, con el preámbulo heredado de Sentry descontado— y con eso **`s5-peso` queda en ROJO: 61,9 KiB contra un techo de 61,25, −0,60 KiB de aire**. **No se tocó el invariante**: un presupuesto se sube UNA vez, con tres cosas en la mano (§6.5 de B4-A), y es del dueño. El recibo está en `docs/rediseno/outputs/b7/peso-del-arreglo.json`. La mayor parte del costo (1.575 B) es un chunk NUEVO que webpack izó al montar el proveedor en el layout: es la frontera real de la lección §6.4 de B4-A, «partir cuesta bytes sólo si la pieza es COMPARTIDA». La alternativa que la evitaría —montar el proveedor abajo, en `CompuertaDelHome`— dejaría `/v3/motion` sin el arreglo, o sea el defecto de vuelta en una ruta.

    ── ⛔ **DOS DEFECTOS CERRADOS SIN ARREGLAR, CON LA MEDICIÓN QUE LOS DESMIENTE**

    **`D2` — «el pin de Servicios no pinea en ningún perfil»: NO SE REPRODUCE.** El pin anda. Medido con scroll real y los dos bordes bisectados a 2 px: **2.158 px de recorrido a 1920** (pegado entre `scrollY` 11.882 y 14.040), 1.798 a 1440 y 1.502 a 1025 — el mismo orden que B1 publicó cuando lo dio por bueno. Y `git diff 8ab34b36 HEAD -- servicios/geometria.ts servicios/Servicios.tsx` devuelve **vacío**: no cambió una línea desde entonces. No hubo regresión.

    **El LCP móvil — «una de dos corridas cruza los 2.500 ms»: NO SE REPRODUCE.** Mismo instrumento (las tres fuentes de `scripts-b4/a-observador.ts`), build de producción propio, 375 con el preset móvil: **2.084 · 2.132 · 2.156 · 2.204 ms — 4 corridas de 4 abajo del techo**, mediana **2.144** contra los 2.378 de B4-B. El elemento LCP sigue siendo el `<h1>` del hero, texto, 34.892 px². Y **diferir el JS no es la palanca**: bloqueando los 25 chunks (350,0 KiB) el techo de contención mide 20 ms con una dispersión de 56 ms en un solo brazo, o sea **indistinguible de cero con n = 3**. Sentry no se difiere y eso sigue cerrado.

    ── 🔴 **LA TRAMPA ESTRUCTURAL QUE PRODUJO `D2`, Y QUE VA A VOLVER A ENGAÑAR — con su método**

    **`Servicios` tiene DOS `sticky` anidados y sólo uno pinea.** `Seccion.tsx` emite, para una sección `pinneada: 'siempre'`, un envoltorio `w-full sticky top-0 min-h-svh`; adentro, `Servicios.tsx` pone su `Bloque` con el alto de la sección ENTERA, así que ese envoltorio **mide lo mismo que su padre y su rango de pegado es cero por construcción, en todos los perfiles y para siempre** (3.240 de 3.240 a 1920; 2.370,44 de 2.370,44 a 1024). No está roto: nunca tuvo recorrido. El pin real es el hijo de `ServiciosEnSecuencia`, que mide una pantalla dentro de un contenedor de tres.

    > **EL DISCRIMINADOR, Y VALE PARA CUALQUIER `sticky` DEL REPO.** Un `sticky` son **dos** elementos: **el hijo que se pega** y **el padre que le da recorrido**. La cifra que decide es `recorrido disponible = alto del padre − alto propio`. **Medir sólo la posición del hijo devuelve el MISMO cero en los dos casos** —el que está roto y el que nunca tuvo recorrido—, así que un cero sin el recorrido disponible al lado no significa nada. B4-B cayó ahí **con Fase 0, con banco y con controles positivos**, y publicó un defecto rojo que no existía.

    ⚠️ **Y una segunda vuelta del mismo pozo, encontrada por el verificador adversarial de este bloque:** el censo leía el alto del hijo **una sola vez, con el scroll en cero**, y este hijo **crece a mitad del pin** —de 774,55 a 798,55 px a 1025×768, porque la secuencia cambia de servicio—. Con el alto estático el desborde daba **6,55 px**; medido en cada parada del barrido da **30,55 px**, casi cinco veces, y la pérdida real de recorrido es el **2,21 %** y no el 0,43 %. **Una altura que puede cambiar durante el recorrido se mide durante el recorrido.**

    ── ⚠️ **TRES INSTRUMENTOS DE ESTE MISMO BLOQUE FALLARON DE LAS TRES FORMAS QUE EL BLOQUE VINO A CAZAR**

    Los encontró la verificación adversarial, no sus autores, y por eso van escritos:

    · **Una TAUTOLOGÍA con forma de afirmación.** `b-pin-comprobaciones.ts` afirmaba «lo que el pin pierde de recorrido es exactamente lo que el hijo desborda» con los dos lados sacados de las MISMAS dos alturas del censo: el término se cancelaba y el predicado se reducía a `padre === pasos × V`, que ya afirmaba la fila de arriba. **No podía ponerse en rojo**, y había reemplazado a una afirmación que sí daba rojo. Hoy los dos lados salen de mediciones independientes —el alto por caja, parada por parada; el rango por scroll real bisectado— y **tiene un control que la hace fallar a propósito**: con el alto alterado 40 px, roja.
    · **Una MEDICIÓN QUE MEZCLA DOS CORRIDAS EN SILENCIO.** `c-palancas.ts` leía la geometría del JSON versionado y los píxeles de `os.tmpdir()`, que se pisa en cada corrida y no llevaba identidad: re-correrlo cruzaba **la caja de una corrida con los píxeles de otra** sin fallar ni advertir. El agregado caía de 22.387 a 16.777 píxeles de glifo y tres de los cinco techos cambiaban solos; **el cuarto coincidía por casualidad, que es lo peor, porque hacía que la corrida corrupta pareciera una reproducción.** Arreglado con un sello de contenido en los dos artefactos (`scripts-b7/c-sello.ts`) que **tira**. La columna «antes» de las palancas **se retiró**: no se puede re-derivar, y un número que no se puede volver a producir no se publica como medición.
    · **UN ARGUMENTO CON FORMA DE MEDICIÓN QUE NO SEPARABA LAS DOS HIPÓTESIS.** Un docblock del banco afirmaba que «24,0 px por evento exactos es scroll NATIVO y no un motor que interpola». `/v3` construye Lenis con `wheelMultiplier: 1`, así que **con** el motor el total también es `n × 24`: medido, 23,968 px/evento con motor y 23,968 sin él. Lo que discrimina es el atributo `data-v3-scroll-suave` y la clase `lenis`, que es lo que se afirma ahora. **Es la familia de «verde por arnés», un piso más abajo.**

    Y un cuarto, del mismo linaje: `volverAlTope` comprobaba `window.scrollY === 0` —la POSICIÓN— cuando lo que el brazo necesitaba era el OBJETIVO interno del motor. Medido, un brazo llegaba a arrancar respondiendo **76,54 px/evento** contra los 24 del arranque en frío, arrastrando el objetivo del anterior, **con el guardián diciendo que todo estaba bien**. Hoy se vuelve al tope por el mismo canal que después se conduce, y el guardián mide **la respuesta**, no la posición.

    ── **`D-B5.5` — el cuadro de 26,7 ms: NO SE REPRODUCE, y la sospecha del instrumento queda descartada**

    Con el guardián arreglado, seis brazos (tres conduciendo con `scrollBy` y tres con eventos de rueda reales), los seis pasando por `y ≈ 10.500` y el lector demostrado sensible —vio los 40 ms inyectados en el cuadro 200—: **cero cuadros por encima de 20 ms en los seis**, con el peor de cada uno entre 13,5 y 13,7 ms, o sea un cuadro normal. No es del instrumento ni del producto: hoy no está.

    ── **`D-B5.1` — el cuerpo del diferencial: MEJORA MEDIDA, y NO alcanza**

    De **34,72 % a 33,61 %** de píxeles bajo AA a 1440 y de **11,25 % a 8,05 %** a 1920, con la misma masa de glifo (22.387 y 11.402 px: ninguna columna es verde por vacío) y el peor píxel subiendo de 1,00:1 a 1,91:1. La palanca fue la POSICIÓN del testimonio; **las otras tres están medidas y no alcanzan** —tamaño contra el umbral de 3:1 deja 7,26 % abajo; negro puro, 21,66 %; registro invertido, 80,91 %—. **No reabre `D-B5.4`**: el techo del paralaje lo siguen poniendo las tarjetas, que no se movieron. **Queda abierto**, y lo que lo cerraría (una superficie detrás del texto o cambiar la pared de la sala) cae en zona prohibida.

    ── **LO DIFERIDO, con su razón**

    **De la zona prohibida:** `D3` (abajo de 1025 no queda ningún pin del recorrido — el arreglo es composición en `_secciones/trabajos/`), `D6` (las tres áreas de toque de 20 px de alto, mismo archivo), el último plano de Trabajos, y `D4`/`D5` (composición y escena abajo del umbral). **Por decisión, no por defecto:** `D8` y `D13`. **Por alcance:** §7.13 y §7.17 (los instrumentos arriba de 300 líneas; el mayor es `_lib/__tests__/tokens.invariant.ts` con **498**, que pasó de 475 en V3-C y **hoy no lo cubre ningún gate** — `LARGOS_HEREDADOS` sólo lista los seis de `_lib/escena/`).

51. ✅ **B8 — LA LUZ MANDA: EL VELO SE BORRA, EL ARCO GANA UNA NOCHE, SEIS SECCIONES VEN LA SALA Y EL CONTRASTE SE ROMPE A PROPÓSITO Y SE DECLARA (2026-09-08).**

    Rama `v3/luz`, worktree `C:\v3-luz`, sobre el merge de `v3/escena-viva` (`cda07be1`). Reporte: `docs/rediseno/outputs/B8-LUZ.md`. **No se tocó** la coreografía de cámara, `anclaje.ts`, `recorrido.ts`, el progreso, el preloader, `/probe-escena` (fuera de `__tests__`), los frozen, ni el contenido o la composición de una sección: `git diff` vacío sobre todos ellos, y `choreography.ts` cambió sólo en que su bloque de `LIGHT_ARC` se mudó a `lightArc.ts`.

    ── 🔴 **EL DIAGNÓSTICO: un velo oscuro sobre una sala de papel blanco da GRIS, nunca negro.** La superficie `oscuro-transparente` de B6-A era una respuesta al problema equivocado: el papel a plena key da 249 y un velo al 0,8 lo deja en ~50. La palanca es la que S11 derivó de la geometría, `nivel = sin(elevación)/sin(36°)`: a nivel 0,08 el mismo papel da 33 en el modelo y la sala del pin de Trabajos midió **0,020–0,031 de luminancia media** en la captura (gris 36–48), contra 0,614 con la sala a nivel 1. El velo, su hoja y sus tres tokens se borraron. Ver §2.5.

    ── **EL ARCO (`lightArc.ts`, `s20-arco.invariant.ts`)**: mediodía → atardecer en la pantalla de scroll en que Trabajos entra (0,4688 → 0,5; 36° → 2,7°) → la noche durante el pin (0,08 hasta 0,625) → amanecer escondido donde la escena no dibuja (→ 0,7375, nivel 0,5) → la mañana de S9 en el ancla del diferencial (0,643 en 0,8525), sostenida hasta el final. Cada parada es un nudo del anclaje o de la visibilidad, leído. Dos reglas nuevas, aprobadas en la PARADA 1: el contraluz atado a la sala por debajo de 0,34 (la fórmula de S6 intacta arriba), y la rampa del paralaje del mouse mudada al atardecer. Un defecto destapado: `SHADOW_FAR` 64 → 200, porque la sombra del logo con el sol a 2,7° mide 168.

    ── **LAS PARTÍCULAS BRILLAN EN LA NOCHE, SIN COLOR** (`particleGlow.ts`): un uniform que sigue al nivel las lleva a un gris lineal de 0,25 por debajo de 0,34, cero exacto con luz; las lejanas bajan y las cercanas suben al mismo gris. Medido: 800–850 por pantalla, ⌀ 3,0 px, pico 118 sobre 45 — contra ≈600, 1,6 px y 47 sobre negro de la referencia. Con 1,0 daban manchas de 220 (el recibo de la calibración quedó en `c-las-ocho-despues-brillo1.json`).

    ── ⚠️ **EL CONTRASTE ROTO, DECLARADO Y NO TAPADO.** `deudaDeclarada()` en `afirmar.ts`: la condición corre intacta, cuenta aparte y el agregado publica «N deudas declaradas». **Ninguna afirmación se aflojó ni se borró**; las 15 de `/probe-escena/__tests__` (de las 18 autorizadas) y las de `_lib` se reescribieron contra la propiedad nueva con el docblock de qué custodiaban (reporte §9). Las seis deudas (`deudas-b8.ts`): D-B8.1 Números (logo detrás + el atardecer adentro de su última pantalla), D-B8.2 Trabajos (asoma a pleno sol; en la noche las partículas quedan debajo de los glifos: 3 de 9 bloques, peor 2,53), D-B8.3 el diferencial (asoma a nivel 0,5 y, donde llena el cuadro —el ancla, a 0,643—, el logo detrás del titular: 5 de 7, peor 1,11; la salida barata de terminar el amanecer en 0,643 antes de reanudar está MEDIDA y NO aplicada por decisión: acorta la entrada bajo AA de 0,075 a 0,028 pero no la elimina, y la causa está en el logo), D-B8.4 el Cierre (tinta clara sobre la mañana: 1,59 por el modelo), D-B8.5 el hero (el logo: 3 de 7, 2,94), D-B8.6 Quiénes somos (el logo: 14 de 17, 1,00). **La causa real de casi todo es el logo detrás del texto, no la luz** — que es el insumo del bloque siguiente.

    ── 🔴 **EL CIERRE ESTÁ ABIERTO EN LA TABLA Y CERRADO POR SU PIE.** `chrome/Pie.tsx` pinta `var(--color-fondo)` por `[data-pieza="pie"]`, que en la invertida es #0E0E0E, y envuelve la sección entera: la sala nunca se vio detrás del Cierre, ni con velo ni sin velo (los 18:1 de B6-A eran el pie). Medidas las tres variantes sobre la sala a 0,643: como está 0 fallas (6,44); oscuro-transparente de verdad **25 de 25 bajo AA** (1,00); papel transparente 13 de 25; papel opaco 0 (4,83). Decisión abierta para el humano: el pie como papel, la sala apagándose al final, o el pie negro como hoy.

    ── **LO QUE NO SE MOVIÓ, POR NAVEGADOR:** el lente de P7 (1427,22 px, origen 688/209 en las tres posiciones del pin), las seis costuras del revelado (tres sin máscara entre transparentes contiguas, `sale` en Trabajos→Servicios, `entra` en Tu panel→diferencial, ninguna en diferencial→Cierre), FPS mediana 75,19 / p05 74,63 / mínimo 73,53 contra 37,45 de B5 y B6-A, y `prefers-reduced-motion` sin arnés en cinco poses (reporte §8).
...
77. **Números queda DESCONECTADA (PORTFOLIO, 2026-09-21) — no hay cifras reales y no las va a haber.** `secciones.ts` la deja en la tabla y `registro.tsx` deja de montar `Numeros` ahí; Trabajos pasa a ocupar su lugar visualmente. ⚠️ **Conserva su scroll —200svh, dos pantallas desde el sprint del cartel— y no baja a `1svh`:** el tramo `números` de la coreografía lleva una pose de cámara entera (0,375 → 0,5) y sin píxeles que recorrer la cámara se teletransporta en la costura con Quiénes somos. Ese recorrido no se puede mudar adentro de Trabajos —un tramo sólo avanza sobre el scroll de SU sección; ver `PANTALLAS_DE_NUMEROS`—, así que lo lleva ella, vacía: lo que se ve mientras dura es la sala moviéndose. El componente, sus archivos y su `PEDIDO` no se tocaron: reconectarla es una línea en cada uno de esos dos archivos. Responde parte del punto 7 de arriba para esa pantalla: no va a haber contenido.

78. ⚠️ **DEUDA DECLARADA: LAS CLASES DEL CUERPO PINTADO ESTÁN DUPLICADAS ENTRE TRABAJOS Y SERVICIOS (2026-09-21).** El cartel de Portfolio enciende su bajada palabra por palabra. El mecanismo NO se copió de ningún lado: el canal es `CanalDePiezas` con el patrón P3, que vive en `_secciones/_contrato/canales.tsx`, y el partidor de palabras es `_lib/palabras.ts` — los dos compartidos, los dos fuera del lane de Servicios. **Lo único duplicado son cuatro líneas**: el helper que arma la cadena de clases del contenedor a partir de `NIVELES_TIPOGRAFICOS`, porque `CanalDePiezas` recibe clases y no un componente. Existe como `clasesDeNivel` en `servicios/geometria.ts` y como `clasesDelCuerpoDelCartel` en `trabajos/geometria.ts`. **No se unificó a propósito:** el lane de Servicios está abierto con otra sesión reescribiéndolo entero, y un import cruzado entre dos secciones —o peor, un conflicto entre dos worktrees sobre el archivo que se está reescribiendo— cuesta más que cuatro líneas repetidas. **Se unifica cuando cierre ese lane**, subiendo el helper a `_lib/tipografia.ts`, que es de donde los dos ya sacan la tabla. Ninguno de los dos inventa clases: si la tabla cambia, cambian los dos solos.

79. ⚠️ **`perspective` NO LLEGA A LOS NIETOS — y por eso la salida en z de P7 nunca se vio (2026-09-21).** `BloqueConMotor` declara `perspective: calc(100svh · 1,5858)` —el foco de la cámara de la escena— sobre SU div, y en CSS la perspectiva alcanza a los HIJOS de quien la declara y a nadie más. El nombre del proyecto de Trabajos usaba el tramo de salida de P7 (`translateZ: 0 → −1000`) colgando de `CapaDelTunel`, o sea como NIETO: de esos mil píxeles no se veía uno solo y lo único que se leía era el `autoAlpha`. **No había un solo `transform-style: preserve-3d` en todo `src/app/v3/`** (los ocho del repo están en `src/components/`, el sitio viejo). Se arregló poniendo `preserve-3d` en el contenedor del túnel, que extiende el espacio 3D del bloque a sus nietos. **Regla:** cualquier pieza que use `translateZ` para alejarse tiene que ser hija de la perspectiva o colgar de una cadena de `preserve-3d`; si no, el z es un no-op silencioso. Se comprueba midiendo: con el foco en 1.427 px a 900 de alto, `translateZ(−1000)` deja el elemento en **58,8 %** de su tamaño — si la caja no se achica mientras se desvanece, la cadena está cortada.

80. ⚠️ **EL TÚNEL DE ZOOM DE TRABAJOS: LO QUE SE MIDIÓ EN heatbureau.com Y QUÉ SE COPIÓ (2026-09-22).** La referencia se midió con el banco de `scripts-b4/` a 1440 × 900, con un Chrome propio por CDP, ruedas sintéticas (`Input.dispatchMouseEvent`) y un `requestAnimationFrame` que anota la caja en pantalla de cada imagen cuadro por cuadro. Cinco hallazgos, y los cuatro primeros contradicen lo que parece a ojo:

    - **No es inercia, es PERSECUCIÓN.** Un solo golpe de rueda de 120 px desde un estado asentado: **el scroll frena a los 764 ms** —ése es su scroll suave, Lenis, constante de tiempo 142 ms, R² 0,9956— y **la imagen sigue creciendo hasta los 2.658 ms**, quedando 46 % más grande de lo que era al soltar. Son DOS amortiguaciones encadenadas; la que se siente es la segunda, con constante de tiempo **454 ms** (decaimiento geométrico de razón 0,746 cada 133 ms, constante hasta el cuarto decimal sobre nueve muestras). Los «~3 s» que se perciben son eso, no el scroll.
    - **La escala NO es exponencial.** Ajustando el ancho en pantalla contra el scroll, restringido al tramo en que cada imagen crece de verdad, **la recta gana en las seis**: R² 0,96–0,995 contra 0,77–0,84 del ajuste logarítmico. Lo que se lee como exponencial es el relevo, no la curva.
    - **No hay perspectiva.** `perspective: none` y `transform-style: flat` en las siete capas; cero `matrix3d`. Es escala 2D pura — lo cual, leído contra §79, dice que un túnel de fotos no necesita el espacio 3D del bloque.
    - **El relevo y la convivencia.** Nace una cada ~300 px de scroll; al nacer una, **la anterior mide 29–36 % del ancho del cuadro**; **seis imágenes conviven el 68 % del tramo**; ninguna desaparece: llegan a **1,17–1,68 anchos de cuadro** y se quedan ahí.
    - **Todo crece centrado.** El centro de las seis está clavado en x = 713 sobre un cuadro de 1440 durante todo el tramo.

    **Qué se copió y qué no.** Se copiaron las tres cifras de composición —el relevo (0,32 anchos de cuadro), el desborde y el centrado— y la existencia de la persecución. **No se copió la cascada de dos amortiguaciones:** acá hay UNA sola, con el tiempo que el humano pidió, porque lo que él mide al soltar tiene que ser el número que está escrito en el código. Tampoco se copió el scroll suave: `/v3` no tiene Lenis y no se agregó uno.

...
- **`resolveOrgId()`** (`lib/preview.ts:6-20`): ORG\_MEMBER → org de la **sesión**; SUPER\_ADMIN → org de impersonation activa o `null`. **Nunca por URL.** Multi-tenant seguro. ✓  
...
- `PremiumModule` trae `longDescription?` (hoy sin usar) y `validRubros[]` (sin filtrar acá — se muestran todos). Enums status: ACTIVE/COMING\_SOON/DEPRECATED (la page ya excluye DEPRECATED).
...
- Badge admin: `text-[11px] tracking-[0.16em] px-2.5 py-1`; COMING\_SOON amber; **"Premium" rojo+Lock → neutral** (`border-white/10 bg-white/5 text-zinc-300`) para igualar la calma del admin (recomendado; vetoable en approval).  
...
- **NO** se importó `getModuleBySlug` ni se acopló la card al módulo de datos: el brief pidió explícitamente "dict local PRESET\_MODULE\_DETAILS". Se respetó la letra.
...
**Lane:** `lane/services` · **Ruta:** `/dashboard/services` (portal cliente) **Branch:** `lane/services` · **HEAD:** `bded0be3f37b15186e76f653377492cb55250230` **Estado:** cerrado, sin mergear. La integración a main la hace el humano.

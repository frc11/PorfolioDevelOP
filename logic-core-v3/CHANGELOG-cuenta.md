...
- `hardDeleteClient` (SUPER_ADMIN), `getClientDeletionSummary` y `TypeToConfirmDialog` REUSADOS sin modificar. Sin schema.
...
| 4 | **Ticket → borrar end-to-end**: abrir ticket "Solicitud de eliminación de cuenta" como SUPER_ADMIN → ver bloque rojo → "Ir a borrar" → llega a ClientsListClient con el dialog abierto para el cliente correcto | ⏳ pendiente |
